...
timestamp_per_min <- rep("", num_minutes)
steps_per_min <- rep(0, num_minutes)
for (i in 1:num_minutes) {
  timestamp_per_min[i] <- timestamp_per_5_secs[(i - 1) * 12 + 1]
  for (j in 1:12) {
    steps_per_min[i] <- steps_per_min[i] + steps_per_5_secs[(i - 1) * 12 + j]
  }
}

file_name <- "~/Desktop/verisense_count_steps/steps_per_min.csv"
my_data <- data.frame(timestamp_per_min, steps_per_min)
